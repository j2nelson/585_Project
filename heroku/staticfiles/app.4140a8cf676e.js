...
function getCentroidofBlob(src) {
  // let M = cv.moments(src, true);
  let centroid = cv.Point(4, 4);
  // let centroid = cv.Point(M.m10/M.m00, M.m01/M.m00);
...
function processVideo() {
  ...
  // let centroid = getCentroidofBlob(process);
  // if(prevCentroid.x > 0 && prevCentroid.y > 0 && centroid.x > 0 && centroid.y > 0)
  //   connectDots(centroid, prevCentroid);

  cv.imshow("canvasOutput", process);
  stats.end();
